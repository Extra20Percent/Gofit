...
@section('content')
    <section id="layout">
        <h1 style="text-align: center;">EDIT INSTRUKTUR</h1>
        <form action="/instructors/update/{{$instructor->id}}" method="post">
            @csrf

            <label for="inputName">Username</label><br>
            <input type="text" id="inputName" name="username" class="w-100" value="{{$instructor->username}}" readonly><br>
            @if ($errors->has('inputName'))
                <span class="error">{{ $errors->first('inputName') }}</span>
            @endif

            <label for="inputEmail">Email</label><br>
            <input type="text" id="inputEmail" name="email" class="w-100"  value="{{$instructor->email}}" readonly><br>
            @if ($errors->has('inputEmail'))
                <span class="error">{{ $errors->first('inputEmail') }}</span>
            @endif

            <label for="inputPhone">Phone Number</label><br>
            <input type="text" id="inputPhone" name="phone" class="w-100"  value="{{$instructor->phone}}"><br>
            @if ($errors->has('inputPhone'))
                <span class="error">{{ $errors->first('inputPhone') }}</span>
            @endif

            <label for="inputAddress">Address</label><br>
            <input type="text" id="inputAddress" name="address" class="w-100"  value="{{$instructor->address}}"><br>
